...
DB アクセスは Repository 経由（§2.8 ルール1）。
"""
...
def _load_sheet(repo: TenantScopedRepository, case_no: str) -> Optional[m.StrategySheet]:
    """案件の作戦シートを取得する（Repository 経由・無ければ None）。"""
    return repo.get(m.StrategySheet, case_no=case_no)
...
def _compute(repo: TenantScopedRepository, case: m.NegotiationCase) -> ThreeLineResult:
    ...
    rate = RateInputs(market_rate=market, current_price=float(case.current_price or 0), yoy_rate=yoy)
    past = past_settled_prices(repo, case.spec_id, case.case_no)
    auto = calc_auto_lines(rate, plan, past)

    # 保存済みの手修正を重ねる（*_final が入っているラインのみ手修正扱い）。
    sheet = _load_sheet(repo, case.case_no)
...
def save_three_lines(
    ...
    sheet = _load_sheet(repo, case_no)
    if sheet is None:
    ...
    # 確定後の状態で1度だけ算出し、自動値・影響額スナップショット・返却に使い回す（Minor 3）。
    result = _compute(repo, case)
    auto_by_type = {ln.type: ln.auto_value for ln in result.lines}
    eff_by_type = {ln.type: ln.value for ln in result.lines}
    for t in _LINE_TYPES:
        setattr(sheet, f"{t}_auto", auto_by_type.get(t))

    plan_row = plan_for_spec(repo, case.spec_id, case.period)
    if plan_row is not None:
        annual = int(plan_row.annual_volume_kg or (int(plan_row.volume_kg_month or 0) * 12))
        pp = float(plan_row.planned_price or 0)
        sheet.impact_target = int((pp - eff_by_type.get("target", 0)) * annual)
        sheet.impact_landing = int((pp - eff_by_type.get("landing", 0)) * annual)
        sheet.impact_walkaway = int((pp - eff_by_type.get("walkaway", 0)) * annual)

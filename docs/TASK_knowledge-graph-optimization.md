...
「越境ゼロ」が **何にどう効くか** を明記します（2026-07-26 セキュリティ監査 F-9 の是正）。

| 要素 | 判定方法 | 区分 |
|---|---|---|
| `hits` / `citations` | `id` 接頭辞が要求テナントと一致するものだけ残す | **接頭辞フィルタの対象** |
| `graph_context.nodes` | 同上 | **接頭辞フィルタの対象** |
| `graph_context.edges` | `src`・`dst` の**双方**が一致するものだけ残す（片端でも他テナントなら除去） | **接頭辞フィルタの対象** |
| `graph_context.summary_text` | 自然文で `id` を持たないため接頭辞では濾せない。**fail-closed の2段判定**で扱う。<br>①グラフ（node / edge）に越境要素が検出されたら要約**全体を破棄**（同じ汚染グラフから作られた要約は信頼しない）。<br>②要約中に他テナント／所属不明の `id` トークン（`{tenant}:{type}:{pk}`）があれば同様に破棄。 | **接頭辞フィルタの対象外**（別方式で検査） |

> [!warning] 残存リスク（既知の限界）
> `summary_text` に、`id` を伴わずに他テナントの**取引先名などの自然文だけ**が混入し、かつ
> グラフ側には一切の越境要素が無い、という状態は機械的に検出できません。この経路は
> `app/api/strategy.py` 経由で LLM プロンプト → 画面まで届くため、要約の生成元
> （`kre/graph/graph_search.py` の `_summarize`）は**必ずテナントフィルタ済みのノードのみ**から
> 組み立てること。クロステナント補完機能を将来追加する場合は、本節の判定方式を先に見直します。

検証は `backend/tests/kre/test_tenant_isolation.py`（`summary_text` の破棄・保持を含む）で固定しています。
